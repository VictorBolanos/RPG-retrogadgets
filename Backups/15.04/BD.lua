...
function Utils:DrawPlayerStatusBar(player, statusIconsSprite)
    ...
    -- DEBUG: Verificar que el sprite existe
    if not statusIconsSprite then
        print("ERROR: statusIconsSprite is nil!")
        return
    end
    ...
    -- DEBUG: Mostrar debuffs activos
    if player.debuffs then
        for name, turns in pairs(player.debuffs) do
            print("DEBUG: Player has debuff " .. name .. " with " .. turns .. " turns")
        end
    else
        print("DEBUG: player.debuffs is nil")
    end
    ...
    for i, debuffName in ipairs(debuffNames) do
        ...
        if statusData then
            ...
            -- DEBUG
            if isActive then
                print(string.format("DEBUG: Drawing ACTIVE %s at pos [%d, %d], sprite [%d, %d]", 
                    debuffName, debuffPositions[i], y, statusData.sprite[1], statusData.sprite[2]))
            end
